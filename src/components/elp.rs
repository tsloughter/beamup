...
fn asset_prefix() -> Result<regex::Regex> {
    let otp_major_vsn = match config::get_otp_major_vsn() {
        Ok(otp_major_vsn) => otp_major_vsn,
        Err(_) => "".to_string(),
    };

    match (std::env::consts::ARCH, std::env::consts::OS) {
        ("x86_64", "linux") => {
            Regex::new(format!("elp-linux-x86_64-unknown-linux-gnu-otp-{otp_major_vsn:}").as_str())
                .wrap_err("Failed to create asset regex")
        }
        ("aarch64", "linux") => {
            Regex::new(format!("elp-linux-aarch64-unknown-linux-gnu-otp-{otp_major_vsn:}").as_str())
                .wrap_err("Failed to create asset regex")
        }
        ("x86_64", "macos") => {
